...
int main(int argc, char *argv[])
{
	...
	error = exact_close(to_fd);
	if (error < 0)
	{
		exact_close(from_fd);
		exit(100);
	}
	error = exact_close(from_fd);
	if (error < 0)
		exit(100);
	return (0);
}
...
/**
 * exact_close - A functionthat closes a file and prints error when closed file
 * @description: Description error for closed file
 * Return: 1 on success, -1 on failure
 */
int exact_close(int description)
{
	int error;
